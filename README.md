...
假设我们有个pdf每一页代表一个部门的人员，有一个部门的人数较多超出一页，可能导致分块失效找不到。

固定长度分块

基于句子分块

结构化分块 md. html.   一级标题二级标题

递归字符(先按短落，段落长按句子划分，句子划分长按词语划分)
...
‌**LangChain4j的核心组件围绕大模型应用开发的全流程设计，主要包括语言模型（Model）、记忆（Memory）、工具（Tool）、链（Chains）、检索增强生成（RAG）和输出解析器（Output Parsers）六大模块**‌。

------

这些组件如同“积木”，让开发者能灵活搭建AI应用：

1. ‌**语言模型（Language Model）**‌
   框架的“大脑”，负责文本生成与推理。支持OpenAI、百度文心、阿里通义千问、本地模型等主流LLM，通过统一API实现模型自由切换 。
2. ‌**记忆（Memory）**‌
   管理对话上下文，实现多轮交互。`ChatMemory`可存储历史消息，支持会话隔离与Redis持久化，确保模型“记得你说过什么” 。
3. ‌**工具（Tool）**‌
   扩展模型能力，使其能调用外部API或执行Java方法。例如，通过`@Tool`注解将天气查询、数据库操作封装为可调用函数，实现“模型驱动代码” 。
4. ‌**链（Chains）**‌
   将多个组件串联成工作流。例如构建“文档问答链”：检索→拼接提示词→调用模型→输出答案，实现复杂逻辑的自动化执行 。
5. ‌**检索增强生成（RAG）**‌
   提升模型专业性与准确性。通过`EmbeddingStore`将私有数据向量化存储，查询时检索相关片段并注入上下文，让回答“有据可依” 。
6. ‌**输出解析器（Output Parsers）**‌
   将模型输出结构化。例如将自然语言解析为JSON、XML或自定义对象，便于程序进一步处理，避免“文本乱炖” 。
